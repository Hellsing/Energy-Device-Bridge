"""Tests for Energy Device Bridge config and options flows."""
...
async def test_config_flow_happy_path(hass: HomeAssistant) -> None:
    ...
    assert (
        result["options"][CONF_ZERO_DROP_POLICY]
        == ZERO_DROP_POLICY_IGNORE_ZERO_UNTIL_NON_ZERO
    )
...
async def test_config_flow_accepts_missing_power_source(hass: HomeAssistant) -> None:
    ...
    assert (
        result["options"][CONF_NOTIFY_ON_LOWER_NON_ZERO]
        == DEFAULT_NOTIFY_ON_LOWER_NON_ZERO
    )
    assert (
...
@pytest.mark.asyncio
async def test_reconfigure_source_is_not_supported(hass: HomeAssistant) -> None:
    """Config flow no longer exposes a reconfigure step."""
    await _init_integration(hass)

    with pytest.raises(UnknownStep):
        await hass.config_entries.flow.async_init(
            DOMAIN,
            context={"source": "reconfigure", "entry_id": "any-entry-id"},
        )
...
async def test_options_flow_updates_entry_from_gear_path(hass: HomeAssistant) -> None:
    ...
    assert (
        entry.options[CONF_ZERO_DROP_POLICY]
        == ZERO_DROP_POLICY_IGNORE_ZERO_UNTIL_NON_ZERO
    )
...
async def test_config_flow_rejects_invalid_entities(hass: HomeAssistant) -> None:
    ...
    await _init_integration(hass)
    hass.states.async_set(
        "sensor.good_power", 5, {"unit_of_measurement": UnitOfPower.WATT}
    )
    hass.states.async_set(
        "sensor.bad_energy", "nope", {"unit_of_measurement": "widgets"}
    )
...
@pytest.mark.asyncio
async def test_config_flow_rejects_non_wh_kwh_and_non_w_kw_units(
    hass: HomeAssistant,
) -> None:
...
async def test_config_flow_accepts_w_and_wh_units(hass: HomeAssistant) -> None:
    ...
    await _init_integration(hass)
    hass.states.async_set(
        "sensor.src_power", 500, {"unit_of_measurement": UnitOfPower.WATT}
    )
    hass.states.async_set(
        "sensor.src_energy", 5000, {"unit_of_measurement": UnitOfEnergy.WATT_HOUR}
    )
...
async def test_config_flow_accepts_generic_sensor_without_device_class(
    ...
    await _init_integration(hass)
    hass.states.async_set(
        "sensor.generic_power", 300, {"unit_of_measurement": UnitOfPower.WATT}
    )
    hass.states.async_set(
...
def test_translation_files_and_runtime_localization_contract() -> None:
    ...
    for lang in (en, de):
        ...
        assert lang["selector"]["zero_drop_policy"]["options"][
            "ignore_zero_until_non_zero"
        ]
...
async def test_user_flow_sets_unique_id(hass: HomeAssistant) -> None:
    ...
    await _init_integration(hass)
    hass.states.async_set(
        "sensor.test_power", 130, {"unit_of_measurement": UnitOfPower.WATT}
    )
    hass.states.async_set(
        "sensor.test_energy",
        EnergyConverter.convert(
            7500, UnitOfEnergy.WATT_HOUR, UnitOfEnergy.KILO_WATT_HOUR
        ),
        {"unit_of_measurement": UnitOfEnergy.KILO_WATT_HOUR},
    )
